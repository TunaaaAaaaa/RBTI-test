...
import { calculateRbtiResult, type TestResult } from "@/lib/rbti-scoring";
...
export default function RbtiResultPage() {
  ...
  useEffect(() => {
    try {
      const raw = sessionStorage.getItem("rbti_data");
      if (!raw) { router.push("/"); return; }
      setResult(calculateRbtiResult(JSON.parse(raw).answers, JSON.parse(raw).hidden));
    } catch { router.push("/"); }
  }, [router]);
  ...
  const generatePoster = useCallback(async () => {
    ...
    ctx.fillText("RBTI · 研究员行为类型测试", W / 2, f(55));

    // Emoji placeholder for character
    ctx.font = `${f(300)}px system-ui`;
    ctx.fillText("🧪", W / 2, f(380));
    ...
    ctx.fillStyle = "#1c1917";
    ctx.font = `900 ${f(28)}px system-ui, sans-serif`;
    ctx.fillText("你是什么类型的研究员？", ctaX, bottomY + f(50));

    ctx.fillStyle = "#2563eb";
    ctx.font = `900 ${f(22)}px system-ui, sans-serif`;
    ctx.fillText("扫码测试你的科研人格", ctaX, bottomY + f(90));

    ctx.fillStyle = "#a8a29e";
    ctx.font = `${f(18)}px system-ui, sans-serif`;
    ctx.fillText("30 道题 · 15 种科研人格", ctaX, bottomY + f(160));

    // Footer
    ctx.textAlign = "center";
    ctx.fillStyle = "#d6d3d1";
    ctx.font = `${f(16)}px system-ui, sans-serif`;
    ctx.fillText("RBTI · 研究员行为类型测试 · 仅供娱乐", W / 2, H - f(30));
  ...
  if (!result) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-[#eff6ff] to-[#f0f9ff]">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-4 border-blue-400 border-t-transparent mx-auto mb-4" />
          <p className="text-stone-400 text-sm">正在计算你的科研人格...</p>
        </div>
      </div>
    );
  }
  ...
  const siteUrl = getSiteUrl();
  const shareText = `我在 RBTI 研究员人格测试中测出了【${p.code} · ${p.name}】！\n「${p.motto}」\n匹配度 ${similarity}%\n\n你是 SCHOLAR 还是 SLOTH？来测测 👉 ${siteUrl}`;
  ...
  const handleSavePoster = () => {
    ...
    a.href = posterUrl;
    a.download = `RBTI-${p.code}.png`;
    a.click();
  };

  const models = ["R", "M", "E", "C", "P"];

  return (
    <main className="min-h-screen bg-gradient-to-b from-[#eff6ff] to-[#f0f9ff]">
      <div className="grain-overlay" />

      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-lg border-b border-blue-100/50">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          <button onClick={() => router.push("/")} className="text-stone-400 hover:text-blue-500 transition text-sm font-medium">← 首页</button>
          <span className="font-mono font-black text-blue-500 text-sm">RBTI</span>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="slide-up">
          {/* Main Card */}
          <div className="rounded-3xl overflow-hidden shadow-2xl mb-8 border border-blue-100" style={{ background: `linear-gradient(160deg, ${p.color}08, ${p.color}18, ${p.color}08)` }}>
            <div className="p-10 text-center relative">
              ...
              <div className="inline-flex items-center gap-2 bg-white/70 backdrop-blur px-5 py-2.5 rounded-full border border-blue-100">
                <span className="text-xs text-stone-400">匹配度</span>
                <span className="text-3xl font-black" style={{ color: p.color }}>{similarity}%</span>
              </div>
            </div>
            <div className="bg-white/60 backdrop-blur px-8 py-6 border-t border-blue-50">
          ...
          {matchDetails.length > 0 && (
            <div className="bg-white rounded-2xl shadow-lg p-6 mb-5 border border-blue-50">
              ...
              <div className="space-y-2.5">
                {matchDetails.map((m, i) => (
                  <div key={m.code} className="flex items-center gap-3">
                    <span className={`text-xs font-black w-6 h-6 rounded-lg flex items-center justify-center ${i === 0 ? "bg-blue-100 text-blue-600" : "bg-stone-100 text-stone-400"}`}>{i + 1}</span>
                    ...
                    <div className="w-20 h-2 bg-blue-100 rounded-full overflow-hidden">
                      <div className="h-full rounded-full bg-gradient-to-r from-blue-400 to-indigo-400" style={{ width: `${m.similarity}%` }} />
                    </div>
                  ...
                  return (
                    <div key={m}>
                      ...
                      <div className="space-y-3 pl-1">
                        {modelDims.map((dim) => (
                          <div key={dim.code} className="border-l-2 border-blue-100 pl-4">
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-xs font-medium text-stone-600">{dim.code} {dim.name}</span>
                              <span className={`text-[10px] font-black px-2 py-0.5 rounded-md ${dim.level === "H" ? "bg-blue-100 text-blue-600" : dim.level === "M" ? "bg-indigo-50 text-indigo-600" : "bg-stone-100 text-stone-400"}`}>{dim.level}</span>
                            </div>
                            <div className="h-1.5 bg-blue-50 rounded-full overflow-hidden">
                              <div className="h-full rounded-full transition-all duration-1000 bg-gradient-to-r from-blue-400 to-indigo-400" style={{ width: `${dim.percentage}%` }} />
                            </div>
                          </div>
                        ))}
                      </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-5">
            <div className="bg-white rounded-2xl shadow-lg p-5 border border-blue-50">
              <h4 className="font-black text-sm mb-3 text-blue-600">💪 优势</h4>
              <ul className="space-y-2">{p.strengths.map((s, i) => <li key={i} className="text-xs text-stone-600 flex items-start gap-2"><span className="text-blue-400 mt-0.5 font-bold">✓</span>{s}</li>)}</ul>
            </div>
            <div className="bg-white rounded-2xl shadow-lg p-5 border border-blue-50">
              <h4 className="font-black text-sm mb-3 text-indigo-600">⚠️ 注意</h4>
              <ul className="space-y-2">{p.weaknesses.map((w, i) => <li key={i} className="text-xs text-stone-600 flex items-start gap-2"><span className="text-indigo-400 mt-0.5 font-bold">!</span>{w}</li>)}</ul>
            </div>
          </div>
          ...
          {/* Share */}
          <div className="share-card rounded-2xl shadow-2xl p-6 text-center text-white mb-5 relative overflow-hidden" style={{ background: `linear-gradient(135deg, #2563eb, #4f46e5)` }}>
            ...
            <div className="relative z-10">
              <h3 className="font-black text-lg mb-2">分享你的科研人格</h3>
              <p className="text-white/60 text-xs mb-4">让更多研究员发现自己的类型</p>
              ...
              <div className="flex gap-3 justify-center flex-wrap">
                <button onClick={handleCopy} className="bg-white text-blue-600 px-5 py-2.5 rounded-full font-black text-sm hover:bg-blue-50 transition-all active:scale-95 shadow-lg">
                  {copied ? "✓ 已复制！" : "📋 复制文案"}
                </button>
          ...
          <div className="flex gap-3">
            <button onClick={() => { sessionStorage.removeItem("rbti_data"); router.push("/rbti-test"); }}
              className="flex-1 bg-white text-blue-500 border-2 border-blue-300 px-6 py-3 rounded-full font-black text-sm hover:bg-blue-50 transition-all">重新测试</button>
            <button onClick={() => router.push("/")}
              className="flex-1 bg-blue-500 text-white px-6 py-3 rounded-full font-black text-sm hover:bg-blue-600 transition-all shadow-lg">回到首页</button>
          </div>

          <div className="mt-10 text-center text-[10px] text-stone-300 pb-8 space-y-1">
            <p className="font-mono">RBTI · 研究员行为类型测试 · 仅供娱乐</p>
            <p className="font-mono">作者 <a href={AUTHOR_URL} target="_blank" rel="noopener noreferrer" className="underline underline-offset-2">程序员鱼皮</a></p>
          </div>
        </div>
      </div>
    </main>
